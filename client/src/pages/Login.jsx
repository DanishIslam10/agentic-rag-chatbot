...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await axios.post(`${import.meta.env.VITE_SERVER_ENDPOINT}/auth/login`, formData,
      {
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-linear-to-tr from-[#9b5de5] via-[#00f5d4] to-[#00bbf9] px-4 font-sans">
      ...
      <div className="relative w-full max-w-md rounded-3xl border border-white/40 bg-white/85 p-8 shadow-2xl backdrop-blur-md">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h2 className="mt-6 text-xl font-bold text-slate-800">Welcome Back</h2>
        </div>
          ...
          {/* <div className="flex items-center justify-between px-1 text-xs">
            <label className="flex items-center gap-2 font-semibold text-slate-600 cursor-pointer select-none">
              <input
                ...
                onChange={handleChange}
                className="h-4 w-4 rounded border-slate-300 text-teal-500 focus:ring-teal-400/20"
              />
              Remember me
            </label>
            <a href="#forgot" className="font-semibold text-teal-500 hover:underline">
              Forgot Password?
            </a>
          </div> */}
        ...
        <div className="mt-6 text-center text-xs font-semibold text-slate-500">
          ...
          <a href="/" className="text-teal-500 transition-all hover:underline">
            Sign Up
          </a>
        </div>
